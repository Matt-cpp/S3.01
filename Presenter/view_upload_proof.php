...
if ($proofId > 0) {
    ...
    if ($saved === '') http_err(404, 'Fichier introuvable.');
...
<head>
    ...
    <style>
        ...
        img, iframe, object { max-width:100%; max-height:100%; box-shadow:0 0 20px rgba(0,0,0,0.5); background:#222; }
        .bar { position:fixed; top:0; left:0; right:0; padding:8px 12px; background:#1e1e1e; font:14px/1.4 sans-serif; }
        .bar a { color:#7fb0ff; text-decoration:none; }
    </style>
</head>
<body>
<div class="bar">
    ...
    &nbsp;|&nbsp;<a href="<?= htmlspecialchars($self . '?' . http_build_query(array_diff_key($_GET, ['raw'=>1])), ENT_QUOTES, 'UTF-8') ?>">Rafraîchir</a>
</div>
