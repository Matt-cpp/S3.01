...
function initSubjectChart() {
  ...
  const data =
    statsData.subjects && Object.keys(statsData.subjects).length > 0
      ? statsData.subjects
      : defaultSubjects;
...
function initEvaluationSubjectChart() {
  const ctx = document.getElementById("evaluationSubjectChart");
  if (!ctx) return;

  // Default data if not provided
  const defaultEvaluationSubjects = {
    "INFFIS2-DEV OBJETS": 5,
    "INFFIS1-PROGRAMMATION": 4,
    "INFFIS1-DEV WEB": 3,
    "INFFIS1-INTRO BDD": 2,
    "INFFIS2-BDD AVANCEES": 2,
  };

  const data =
    statsData.evaluationSubjects &&
    Object.keys(statsData.evaluationSubjects).length > 0
      ? statsData.evaluationSubjects
      : defaultEvaluationSubjects;
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: function (context) {
              return `${context.raw} absence(s) en évaluation`;
            },
          },
        },
      },
...
function initMonthlyChart() {
  ...
  const data =
    statsData.monthly && statsData.monthly.labels
      ? statsData.monthly
      : defaultMonthly;
...
function initSubjectTrendsChart() {
  ...
  const data =
    ...
    statsData.subjectTrends.labels &&
    statsData.subjectTrends.labels.length > 0
      ? statsData.subjectTrends
      : defaultTrends;
...
function updateStudentModal(data) {
  ...
  document.getElementById(
    "detail-student-name"
  ).textContent = `Statistiques de ${data.name}`;
  document.getElementById(
    "detail-student-id"
  ).textContent = `Identifiant: ${data.student_number}`;
